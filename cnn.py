...
y_data = concentration['concentration_lactate'] * 1000
...
def init_model():
    ...
    # without PCA
    # x = tf.keras.layers.Conv1D(filters=1, kernel_size=32, use_bias=False)(input)
    # x = tf.keras.layers.AveragePooling1D(pool_size=2)(x)
    # x = tf.keras.layers.Conv1D(filters=1, kernel_size=16, use_bias=False)(x)
    # x = tf.keras.layers.AveragePooling1D(pool_size=2)(x)
    # x = tf.keras.layers.Conv1D(filters=1, kernel_size=8, use_bias=False)(x)
    # x = tf.keras.layers.AveragePooling1D(pool_size=2)(x)
    # x = tf.keras.layers.Conv1D(filters=1, kernel_size=4, use_bias=False)(x)
    # x = tf.keras.layers.AveragePooling1D(pool_size=2)(x)
    # x = tf.keras.layers.Conv1D(filters=1, kernel_size=2, use_bias=False)(x)
    # x = tf.keras.layers.AveragePooling1D(pool_size=2)(x)

    # with PCA and 80 components
    x = tf.keras.layers.Conv1D(filters=1, kernel_size=32, use_bias=False)(input)
    x = tf.keras.layers.Conv1D(filters=1, kernel_size=16, use_bias=False)(x)
    x = tf.keras.layers.Conv1D(filters=1, kernel_size=8, use_bias=False)(x)
    x = tf.keras.layers.Conv1D(filters=1, kernel_size=4, use_bias=False)(x)
...
model = init_model()
model.summary()
history = model.fit(x_train, y_train, epochs=8000, validation_data=(x_test, y_test))
